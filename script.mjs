...
window.onload = function () {
  ...
  form.addEventListener("submit", function(event) {
    ...
    const revisionDates = calculateRevisionDate(pickedDate);

    const allDates = [date, ...revisionDates];

    const allEntries = allDates.map((d) => ({
      topic,
    ...
    const combinedData = [...existingData, ...allEntries].sort(
      (a, b) => new Date(a.date) - new Date(b.date)
    );

    clearData(userId);

    addData(userId, combinedData);
